...
| `tools/lane_status.py` | Read committed `lane/*` refs without opening sibling worktrees; reports unintegrated advisory claims, excludes only full-hash canonically reviewed dispositions from `--pending-only`, and preserves their reason and decision commit in full reports (ADR 0011). | [`docs/CONTRIBUTING.md`](CONTRIBUTING.md) `## Lane helpers` |
