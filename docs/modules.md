...
The remaining unnamed code is `0x2E0`, **2.4% of the corridor**, in one
contiguous run:

| ROM | Size | Note |
|---|---|---|
| `0x743A0`–`0x74680` | `0x2E0` | Between the exact EEPROM read TU and the exact `__osEepStatus` tail |
...
against a single build; at 97.6% against five it is carrying more weight than
it has earned, and the plainest reading of `0x2E0` that five libultra builds do
...
Disassemble `0x743A0`–`0x74680` before running the finder over anything else.
