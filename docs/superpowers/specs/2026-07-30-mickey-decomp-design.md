...
- **Clean-room policy:** see `docs/CLEANROOM.md` for the full policy on permitted/prohibited knowledge sources.
...
- **Local assets:** DKR repo at `~/Desktop/dev/Diddy-Kong-Racing` (100% matched source, working splat 0.35.2 venv, macOS ido-recomp binaries, asm-differ/m2c/asm-processor/permuter, `tools/decomp-workbench`); public Dinosaur Planet decomp at `~/Desktop/dev/dp64` (same Rare DLL/reloc system) — consulted only for its documentation of the binary reloc-table *format* (field offsets, relocation types, table structure), never as a symbol or code source (narrow exception defined in `docs/CLEANROOM.md`). The leaked SFA debug build is not a permitted source under any circumstance and is not used.
...
- **Phase 3 — Overlay system.** Reverse the reloc-table format of the runtime linker by reading the retail ROM directly; the public Dinosaur Planet decomp project's documentation/tooling may be consulted to understand the binary reloc-table *format* (field offsets, relocation types, table structure) since it shares the same DLL mechanism — never as a source of symbol names or code (see `docs/CLEANROOM.md`). JFG repo is a permitted source for its overlay implementation (public retail-ROM decomp). Teach splat the `front`/`track`/`clone` segments, integrate overlay builds + relocation emission into the Makefile. Success: overlays rebuild byte-identically.
