...
- **Phase 3 — Overlay system.** Reverse the reloc-table format of the runtime linker (format reference only, no symbol import: Dinosaur Planet DLL tooling's public documentation of the reloc-table layout; JFG repo for its overlay implementation), teach splat the `front`/`track`/`clone` segments, integrate overlay builds + relocation emission into the Makefile. Success: overlays rebuild byte-identically.
