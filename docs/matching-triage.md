...
| `overlay1FindBestRecord` | o1 tail TU | **P** | retained isolated evidence is exact-sized at 30 words/frame zero with one 12-site `a1/a3` carrier exchange, first `+0x04` | six-run declaration/scope ladder; combine only strict improvements |
...
| `func_overlay_073_F0000000_18CAAC0` | o73 standalone | **P** | retained 92/100 words/frame `0x8`; one `a2/a3` pool-color swap, with all eight LOCAL records runtime-proved against data base `+0xF80` | fresh V0, then bounded allocator search |
...
| `overlay40FadeRecords` | o040 | **R** | retained configured candidate is 3/101 register-only words from exact at `+0xC/+0x10/+0x24`, frame `0x8`; the 20-minute blind batch did not improve it | five-run cap: fresh baseline, reverse the `current`/`output` value origin, then declaration order; combine or initialize declarations only after a strict improvement |
...
| `overlay20UpdateObjectResource` | o020 | **P!** | exact 98-word/`0x70`-frame shape and four call sites; eight count/entries-web register words from `+0xB0`; flags, bounded permuter, and trace-guided lifetime forms exhausted | reproof-only V0; park absent a new pool-to-temp mechanism |
