...
| `func_8003A2C8` | menu | **P!** | retained configured full-TU and isolated C agree at 27/32 words, frame zero, with six exact relocations; the five differences are the initial mode-value register web. No linked C artifact survives. Historical forced-color prose reports 31/32, but the remaining memory-first comparison carrier is ring-only and its object is unretained | one fresh configured V0; then park absent a new UGEN/allocator mechanism |
