...
| `overlay1AssignRecordIndex` | o001 tail TU | **F!/W** | retained configured full-TU C is 36/44 words, frame `0x38`; five output stack-home sites and three conditional-store schedule/address sites. Runtime requires three-argument `GetRomlistInfo` plus three `D_1D8C` pairs, while the current alias/literal store is identity-wrong and linked proof is fallback-only | unchanged V0, identity-correct V1, then declaration ladder only if V1 preserves shape; cap eight |
