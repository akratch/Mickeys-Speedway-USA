...
| `func_8003A2C8` | menu | **P!** | retained configured full-TU and isolated C agree at 27/32 words, frame zero, with six exact relocations; the five differences are the initial mode-value register web. No linked C artifact survives. Historical forced-color prose reports 31/32, but its object/trace is unretained | V0, artifact-unrun flag lattice, recreate forced-color diagnostic, then one trace-selected source spelling; cap four |
