...
| `func_80020D8C` | models | **P/W** | retained configured full-TU and isolated C agree at 35/48 words, frame `0x8`, no relocations; thirteen register-only sites comprise a `t3`→`t5` texture-address lane shift plus later frame-value, multiply-result, and `t0`/`t1` count webs. Sole caller passes unused owner/context in `a3`; linked equality is fallback-only | one unchanged configured V0, then one UGEN-trace-led candidate-only-temp removal; otherwise park |
