...
| `func_80028FCC` | main | **F/W** | retained configured full-TU C is 17/27 words/frame `0x18`, first `+0x1C`, with all three calls exact; isolated C omitted canonical `-Wo,-Olimit,100`. Natural reused-result/shared-epilogue source is staged. ORT 663 has no authenticated inbound; linked equality is fallback-only | compile staged form; if not exact retain 119 flags, then one trace-selected lifetime form only after strict gain; cap 121 builds plus trace |
...
| `overlay7DispatchSelection` | o7 tail TU | **P!/W** | retained pre-current-line-layout full-TU C is 55/60 relocation-annotated, 53/60 literal, and 58/60 runtime-normalized, frame `0x20`, with all 13 records. One flags-value carrier differs at `+0x4/+0x10`; ORT 1510 has fourteen calls. Historical route exhaustion is unretained and linked equality is fallback-only | identity-correct current V0, retain 119 flags, one allocator trace, and at most two trace-selected natural scalar forms; cap 122 stock builds plus trace |
| `overlay7CommitSelection` | o7 tail TU | **P!/W** | retained pre-identity-repair C is 66/72 raw and 69/72 runtime-normalized, frame `0x30`; one post-`mathRnd` narrowing carrier differs at `+0xBC/+0xC4/+0xD4`. Current source expresses all 17 records but is uncompiled. ORT 1345 has seventeen calls; historical exhaustion is unretained and linked equality is fallback-only | identity-current V0, retain 119 flags, one allocator trace, and at most two natural narrowing forms; cap 121 stock builds plus trace |
...
| `overlay59PrepareEntry` | o59 standalone | **P!/W** | retained pre-current-alias full-TU/isolated C is 52/62 raw and 53/62 normalized, frame `0x28`, first substantive `+0x54`; nine sites are one carrier. All six identities, no padding/export, and four local calls are authenticated. Policy-clean split-carrier source is staged; linked equality is fallback-only | compile staged form, retain 119 flags, trace once, then one line/scope form only after strict gain; cap 121 builds plus trace |
...
| `func_80045BBC` | diCpu | **F/W** | retained configured full-TU and isolated C agree at 52/60 raw and 58/60 fixed-address-normalized words/frame `0x30`; candidate emits 18/24 static tuples and `+0xBC/+0xC0` are one final-argument FIFO web. Linked equality is fallback-only; sole caller is `func_80045CAC+0x64`, with no export/runtime/overlay inbound | build identity-correct V0, retain 119 flags, one FIFO trace, one natural form and improving-only combination/batch; cap 125 deterministic builds plus one trace |
| `func_8004BA8C` | font | **F/P!** | retained pre-current-layout C is diagnostic 38/46 raw/normalized words/frame `0x30`, first `+0x30`, with all nine tuples exact. Artificial `stackPad`/empty-condition aids are removed; clean V0 is uncompiled and linked equality is fallback-only. ORT 880 has three resident and two overlay callers | clean V0, 119 flags, one allocator trace, a replacing scoped glyph index and one natural lifetime form, improving-only combination; cap 122 stock builds plus one trace, no batch absent a legal gain |
...
| `levelGetCounts` | level | **F/W** | retained pre-HEAD/current-token configured C is 255/259 raw and 256/259 normalized, frame `0x58`, all 37 offsets/types. Raw `+0x50` is the endpoint identity; substantive `+0x13C/+0x148/+0x154` are one carrier. Endpoint-correct source is uncompiled. ORT 518 has one Overlay 18 inbound; linked equality is fallback-only | retain 119 flags, trace once, one carrier form and strict-gain combination; cap 122 builds plus trace before improving-only batch |
...
| `func_8005A948` | models_5B300 | **F/W** | retained isolated C for the earlier body is 83/94 words/frame `0x38`, first `+0x40`, with 13 exact records, but omitted canonical `-Wo,-loopunroll,0`. Current p10's reported 91/94 is unretained and used a redundant boolean wrapper, now removed. Zero runtime records/export; sole caller is `func_8005A7A0+0x104`; linked equality is fallback-only | clean configured full-TU V0, retain 119 flags, one allocator trace, at most one trace-selected natural form; cap 120 builds plus trace |
...
| `func_8002B7AC` | memory | **F/W** | retained policy-clean full-TU/isolated C is 62 versus target 63 words, frame `0x30`, 1/63 positional words, first `+0x04`; all 12 identities agree but no tuple offset does. Linked equality is fallback-only; ORT 593 has sole caller `func_80026FB4+0x5F8` | fresh V0, 119 flags, one allocator trace, JFG-faithful lexical and scoped early-base/later-cursor forms, strict-gain combination; cap 122 deterministic builds plus one trace, no batch absent legal gain |
...
| `overlay80UpdateContact` | o080 standalone | **reproof** | an unretained permuter result claims 180/180 words and a `0x80` frame from a source-faithful body, but no winning C object or linked artifact survives. Canonical exact output is the assembly fallback; retained genuine-C baseline is 142/180 with the expected 20 records. ORT 1290 has one resident inbound at `func_8000AEEC+0x3B0` | replay only the claim source from current canonical state; prove 180 words, frame `0x80`, all 20 identities, no fallback symbol, exact owned range/module/full bin, then disclose its three allocation temps before promotion |
