...
| `func_8003A2C8` | menu | **P!** | retained prior-layout configured full-TU and isolated C agree at 27/32 words, frameless, with six exact relocations; the five differences are the initial mode-value web. ORT 606 and three callers are authenticated. No linked C artifact survives; forced-color 31/32 and route exhaustion are unretained prose | current V0, exactly 119 flags, one allocator trace, at most two natural forms and one improving-only combination; optional improving-only batch, cap 122 deterministic builds |
| `overlay11UpdateMenu` | o11 standalone | **P!/W** | retained prior-layout configured full-TU and isolated C agree at 285/301 raw and 299/301 runtime-normalized words, frame `0x48`, with all 102 sites; only the `+0x138/+0x140` call-live stores remain. The `+0x8` resident identity is repaired; linked equality is fallback-only | current identity-correct V0, exactly 119 flags, one allocator trace, at most two independent natural forms and one improving-only combination; optional improving-only batch, cap 123 deterministic builds |
...
| `overlay31InitializeBuffers` | o31 standalone | **reproof** | current guarded declaration-order replay matches the claimed exact lane source, but no exact C object survives. Prior C is 225/245 with frame `0x40` versus target `0x48` and all 54 static sites; target data identities and C-linked equality remain unproved | compile current V0 once, decode and prove all 54 runtime identities, then prove 245/245, frame `0x48`, owned range, module, and ROM before removing the guard |
...
| `overlay97InitScale` | o97 standalone | **F/P!** | retained prior-layout configured C is 143/144 raw/normalized words, frameless, first/only `+0xD0`, with zero relocations. Candidate derives the values cursor as `a1+0x3E`; target uses equivalent `a3+2`. The isolated three-word row omitted `-Wab,-r4300_mul`; sole caller is `func_8000AA38+0x7C` | current V0, `(u8 *)bounds+2`, independently `&bounds->bounds[1]`, then improving-only narrow scope; four-build cap, no generic permutation |
| `func_8002CF6C` | saves | **F/P!/W** | retained configured full-TU and isolated C agree at 79/88 words, frame `0x48`, all 11 relocations exact; `savedFlag` is colored `a3` versus target FIFO `t1`, with current-byte and final-copy webs rotating behind it. Only V0 has attributable artifacts | V0, retained flag lattice, one allocator trace, then one trace-selected natural carrier/lifetime/line form; cap four |
| `func_80020D8C` | models | **P/W** | retained configured full-TU and isolated C agree at 35/48 words, frame `0x8`, no relocations; thirteen register-only sites comprise a `t3`→`t5` texture-address lane shift plus later frame-value, multiply-result, and count webs. One resident call and eight runtime overlay records across overlays 57, 60, and 82 are authenticated; linked equality is fallback-only. Only V0 has attributable artifacts | V0, retained flag lattice, one allocator trace, then one trace-selected natural web-birth/lifetime/line form; cap four |
...
| `debug_text_width` | diprint | **F/P!/W** | retained prior-layout configured full-TU and selective C are 59/66 raw/normalized words, frame `0x138`, first `+0x38`, with five exact relocations. Target splits current-byte/classification across `v1/v0`; candidate uses `v0` and branch-likely newline lowering. No caller is authenticated; ORT 862 is unreferenced | current V0, retain 119 flags, two dual-carrier forms, one trace-selected lifetime and an improving-only combination, then one improving-only batch; cap 123 retained trials |
...
| `overlay59PrepareEntry` | o59 standalone | **P!/W** | retained configured evidence is 52/62 raw and 53/62 normalized words/frame `0x28`; target loads descriptor values through `v0` and moves them to `a0` in call delay slots, while retained V0 precolors the web as `a0` | identity-correct V0, flag lattice, trace, split value/result carrier, improving-only line/scope probe; cap five |
| `overlay19ClassifyEdge` | o19 standalone | **P!/W** | retained configured full-TU and isolated C agree at 110/120 words, frame zero, with zero static/runtime relocations; six `$v1`/target-`$t3` carrier sites and four final two-load ordering sites remain. Sole authenticated caller is `overlay19FindAdjacent+0xD8`; linked exactness proves the fallback only | fresh unchanged V0 plus the artifact-unrun 119-flag lattice; park if canonical flags remain best |
| `overlay99BuildHeightGrid` | o099 standalone | **F/W** | retained genuine pre-p8 isolated C is 104/114 normalized words, frame `0x28`, with seven final-grid address-web sites plus three negative-magnitude sites and all 29 runtime tuples, but its recipe omitted the TU's explicit `-Wo,-loopunroll,0`. Current p8 reportedly removes the first seven, but no object survives; linked equality is fallback-only | current configured V0, then one dead-argument-lifetime probe and at most one strict-improvement combination; cap three |
...
| `overlay14ResetMode` | o14 standalone | **F/W** | retained pre-identity configured C is 45/56 raw and 49/56 normalized words, frame `0x30`; the extracted target omits eight loader-owned records. Current source annotates the `+0x54` SYMBOL callee as `overlay14ReleaseOwner`, but compiled pre/postprocess identity proof is pending | current identity V0, retained flag lattice, trace, at most three natural `s3`-lifetime forms, then improving-only batch |
| `overlay1FindBestRecord` | o1 tail TU | **P!** | retained isolated and prior-layout full-TU C are 18/30 words, frame zero, with one 12-site `a1/a3` carrier exchange and all four runtime tuples. The isolated recipe omitted `-Wab,-r4300_mul`; current-layout configured proof is pending and linked identity proves fallback only | current V0 plus three declaration/scope forms; stop if all are flat, otherwise combine strict improvements only within a six-run cap |
| `overlay1FindType5ByKey` | o1 start TU | **P!/W** | retained prior-layout configured full-TU and isolated C agree at 22/39 raw/normalized words, frame `0x30`, first `+0x1C`, with 17 coupled register-field sites. Runtime proves outbound `+0x14` to resident `func_8000572C` and sole inbound `overlay1TransitionState+0x40`; V0's proxy is diagnostic | current V0, identity-correct V1, retain 119 flags, one allocator trace, at most two independent natural forms and one improving-only combination; optional improving-only batch, cap 123 deterministic trials |
| `rcpClearZBuffer` | rcpFast3d | **F/W** | retained configured full-TU C from the preceding declaration state is 103/107 words, frame zero, with both relocations exact and four FIFO/web sites at `+0x74..+0x80`; isolated ranking adds seven schedule sites | current-source V0, then one UGEN FIFO/web trace and at most two indicated forms |
...
| `overlay1AssignRecordIndex` | o001 tail TU | **F!/W** | retained prior-layout configured full-TU and isolated C agree at 36/44 words/frame `0x38`, first `+0x1C`; the TU flag omitted by the isolated recipe is inert. Five output stack-home sites and three conditional-store schedule/address sites remain. Runtime requires three-argument `GetRomlistInfo` plus three `D_1D8C` pairs, while V0's alias/literal store exposes only five records | current V0, identity-correct V1, one retained flag lattice, five independent declaration orders, then one trace-selected residual form; cap eight natural forms plus the lattice |
| `overlay40FadeRecords` | o040 | **R!** | retained configured full-TU/isolated C is 98/101 words with register-only sites at `+0xC/+0x10/+0x24`, frame `0x8`, and all ten runtime BSS tuples; resident `func_8000D978 +0x130` is the sole proven runtime-resolved inbound trap site, while linked range/module/ROM proof is fallback-only | V0, reverse the `current`/`output` origin, then one declaration order; stop if both probes are flat, otherwise combine or initialize only strict gains within a five-run cap |
| `overlay74Update` | o074 standalone | **P!/W** | retained configured full-TU and isolated C agree at 94/100 words, frame `0x60`, with one five-site `t3/v0` aggregate-pointer exchange plus one commutative OR; eight runtime records versus four target-static calls, sole inbound identity `func_8000AEEC +0x34C`, and no linked C proof | V0, query ABI, no-argument hit ABI, reversed outer OR; combine only strict gains, cap five |
| `overlay34InitStorage` | o034 | **F** | retained configured full-TU/isolated C is 45/50 raw and 46/50 normalized with exact 50-word/`0x30`-frame shape; target uses `sp+0x1C` and IDO `sp+0x18` at `+0x24/+0x30/+0x78/+0x7C`; eight candidate/runtime relocations | V0, artifact-unrun flag lattice, then one trace-selected source lever; cap four steps |
| `overlay20UpdateObjectResource` | o020 | **P!/W** | retained prior-layout full-TU and isolated C agree at 90/98 raw/normalized words, frame `0x70`, first `+0xB0`. Four sites are the count/entries web and four a separate `value0F`/`start` argument web. Four exact runtime calls resolve to `trackGetTrack`, `func_8000FEEC`, local `overlay20ConfigureResource`, and `sqrtf`; sole inbound is `func_8000AA38+0x2CC` | current V0, retain 119 flags, one trace, one natural form per web and an improving-only combination; one ≤2,000-candidate batch, cap 122 deterministic builds |
| `func_overlay_041_F0001650_1888988` | o041 | **P!/W** | retained configured C is frameless and 49/55 words. Four count-web sites exchange `a1/v1`; the `+0xA0/+0xA4` load order also puts the candidate `D_0` LOCAL LO16 at `+0xA4` instead of runtime `+0xA0`. The other pair is SYMBOL-bound `gOverlay41Slots`; only V0 has attributable artifacts | V0, retained flag lattice, multiplication-operand reversal, then one trace-selected natural count-web form; cap four |
