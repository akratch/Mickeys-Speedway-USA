...
| `overlay1AssignRecordIndex` | o001 tail TU | **F!/W** | historical configured measurement is 36/44 words/frame `0x38`, but only a wrong-recipe isolated C object survives; full-TU equality is fallback. Five output stack-home sites and three conditional-store schedule/address sites remain. Runtime requires three-argument `GetRomlistInfo` plus three `D_1D8C` pairs, while the current alias/literal store is identity-wrong | unchanged configured V0, identity-correct V1, then the five declaration orders and one residual-led probe; cap eight |
