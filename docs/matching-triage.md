...
| `overlay19ClassifyEdge` | o19 standalone | **P!/W** | retained configured full-TU and isolated C agree at 110/120 words, frame zero, with zero static/runtime relocations; six `$v1`/target-`$t3` carrier sites and four final two-load ordering sites remain. Sole direct caller is `overlay19FindAdjacent+0xD8`; linked exactness proves the fallback only | fresh unchanged V0 plus the unrecorded 119-flag lattice; park if canonical flags remain best |
