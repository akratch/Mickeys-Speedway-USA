...
- relocation proof: Candidate and target each have six records and all six offsets/types agree. Runtime metadata authenticates the resident list-getter call at `+0x34`, two local-data HI16/LO16 pairs at `+0x44/+0x48` and `+0x60/+0x68`, and the local wrap call at `+0xAC`. Strict preflight resolves only 1/6 candidate identities because the list getter and two local-data aliases remain generic; five identities therefore remain unproved and linked equality is fallback-only.
...
- next action: Reopen only with new source-authentic evidence for the paired FP-pool and start-carrier allocation mechanism plus a stable mapping for the two local-data identities. Do not repeat this lattice, these nine forms, or the historical generic permutation batch.
