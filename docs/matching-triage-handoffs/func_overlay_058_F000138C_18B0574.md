...
- Attempt five spells the capture through equivalent signed indirect and
  ...
  corresponding unsigned array load reproduces the 1088 witness. No new
  instruction repair or draw mechanism appears.

Two overbroad exploratory cells were rejected during semantic review and are
not admissible evidence: a whole-case unsigned substitution changes a signed
threshold, and a long-pointer read does not preserve the C effective type.
Neither touched tracked source. Their rejection is recorded beside the private
artifacts so their scalar scores cannot become future candidates.
