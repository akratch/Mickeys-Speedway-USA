...
## 2026-09-12 (lane `p14-whale`): independent stores outside the recorded order runs

594 -> 566 masked, raw 750 -> 722, size delta 0, frame 0x138, first
mismatch +0x50. Aligned buckets: byte-exact 3311 -> 3339, naming 198 ->
170, immediate 32 unchanged, structural 86 unchanged. No matching credit:
the candidate remains guarded by `NON_MATCHING`.

The previous move-one fixed point was only a fixed point over the tool's
enumerated runs. `blockclimb.py` terminates a run at a blank or comment line,
and its default minimum run length is three. Each of the three transition
tails has an independent saved-position store separated from the other two
stores by a blank line, so the recorded sweep never compared these orders.

In case 3, restoring the saved position between restoring and decrementing
the other coordinate gains eight words. In cases 12 and 13, restoring the
saved position first gains ten words each. Six individual orders and their
combined candidate were compiled with the configured full-TU command; the
combined score is 566. All stores name distinct overlay globals, neither
saved local is modified, the dependent restore/decrement pair stays ordered,
and no call is crossed. This is a semantic audit of the three changes.

The stock pre-scheduler listing and the fidelity-checked instrumented UGEN
trace place the saved-position load after the multiply/subtract temporary
draws in the old source. Reordering the stores changes that draw order;
the final instruction shapes are unchanged and the three naming residuals
disappear completely. Thus these windows were emission-order questions,
not independent globalcolor decisions. The initial instrumented object has
byte-identical owned text and an identical relocation map to stock output;
its fresh procedure-index record identifies ordinal zero and 430 decisions.

The paired grid/case-10 defect remains open at this checkpoint. A stock
pre-scheduler listing contains no instruction for the top-of-case index
definition: it is eliminated before as1 runs. Moving its source line cannot
fill the first call's delay slot. Six explicit record/name-cursor variants
tested whether removing strength reduction could free that definition;
all regress, with size deltas from minus 28 to plus 32 bytes.

A separate diagnostic map prices 139 offered, equal-cost allocator choices
against the target object directly. Every force records acceptance. These
objects are ignored diagnostics, never source or matching evidence. The
constant-seven web alone is worth seven naming words; the target colour
for constant five is absent from its offer list and a direct force declines.
Further per-window findings will follow in this section.
