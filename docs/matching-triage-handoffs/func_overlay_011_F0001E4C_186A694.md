...
`c89f914b0edf8d272a8f889c48e2dc9f2d046ba3`:
...
- the ABI is `void (s32)`. The sole authenticated caller is the same-overlay
  dispatcher at `overlay:11:+0x984`, ROM `0x18691CC`; the target has no export,
  resident inbound, or cross-overlay inbound relocation;
- configured IDO 5.3 `-O2 -mips2 -32` emits 1,176 bytes / 294 words against
  the 1,180-byte / 295-word target. Both frames are `0x40`; 109 positional
  words match, leaving 186 relocation-masked / 189 raw differences, first
  `+0x10`;
- workbench classifies 18 aligned structural, 18 schedule, 55 register, and
  three constant residuals with six insertions and seven deletions. The
  candidate is one instruction short and the early pool/temp web diverges;
- target runtime and candidate static text surfaces each contain 87
  ...
  Runtime roles are 40 SYMBOL, 38 LOCAL, and nine JUMP records; shifted sites
  and ambiguous candidate identity for `func_overlay_011_F0000000_1868848`
  keep `function_preflight.py` fail-closed;
- the donor scan is exact-negative. Weak top results are BK
...
- the retired carrier-schedule rewrite is not admissible. Prior base-offset,
  ...
  remain closed. This pass did not mutate the body.
