...
Although source names the 13 SYMBOL call endpoints, the fallback-specific
production postprocess still collapses twelve onto one zero carrier. A genuine
C promotion must replace that collapse with 13 distinct typed proxies and give
the BSS identities unique stored-addend aliases.
...
Historical same-body C was reported to emit all ten roles, but no candidate
object/report survives and current tuples await reproof. The fallback target
retains none statically, making the runtime table the identity authority. ORT
...
`+0x13C/+0x140`. Retained diagnostic genuine C emits all eight; the fallback
