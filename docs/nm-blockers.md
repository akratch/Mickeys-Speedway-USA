...
| `func_overlay_074_F00000B8_18CBD58` | `src/overlays/o074/overlay74Update.c` | none | — | Retained configured-flag isolated import is 94/100 words with six register-only sites at `+0xC,+0x68,+0x84,+0x8C,+0x90,+0x124`. All eight runtime tuples agree, including `gOverlay74Flags` as reserved selector `0xFFF`/addend `0x4D6E8` to `D_800D3128`; the assembled target retains only four static calls. Run unchanged V0 plus at most the `func_800291B4(void)` and resident `func_8005776C` ABI forms, then park. |
