...
| `func_overlay_001_F0007B64_1853F44` | `src/overlays/o001/overlay_001_tail.c` | none | — | `overlay1FindBestRecord` retains isolated C at 18/30 words, frame zero, with 12 runtime-normalized `a1/a3` carrier sites from `+0x04` and all four runtime tuples, but the import omitted `-Wab,-r4300_mul`; no configured full-TU C object survives and linked proof is fallback-only. Run configured V0 and three primary declaration/scope forms; stop if flat, otherwise combine strict gains only within a six-run cap. |
