...
| `debug_text_width` | `src/main/diprint.c` | resolved-plateau | `D_8007CE98` | Retained prior-layout configured full-TU and selective C are 59/66 raw/normalized words, frame `0x138`, first `+0x38`, with exact calls to `sprintfSetSpacingCodes` at `+0x18/+0x30`, `vsprintf` at `+0x28`, and the `D_8007CE98` pair at `+0x4C/+0x50`. Five register and two newline-branch words remain; linked equality is fallback-only and unreferenced ORT 862 has no proven caller. Run current V0, retain 119 flags, two dual-carrier forms, one trace-selected lifetime and an improving-only combination; optional improving-only batch, cap 123 retained trials. |
...
| `levelGetCounts` | `src/main/level.c` | resolved-plateau | D_800CF3E0, D_800CF420 | Retained configured full-TU C is 255/259 raw and 256/259 address-normalized words, frame `0x58`, with 37 records. Target endpoint `+0x44/+0x50` names `D_800CF420`; candidate names address-equivalent `D_800CF3E0+0x40`; the other three sites are one `$v0`/target-`$a0` web. The lower isolated score is a reformatted import. Linked equality proves fallback only; Overlay 18 ORT 518 is the sole inbound. Run V0 and the flag lattice, then independent endpoint and trace-selected web probes, their improving-only combination, and one improving-only annotated batch. |
...
| `func_80019DE8` | `src/main/lights.c` | resolved-plateau | `D_800CB290` | Retained configured full-TU C is 19/63 positional words with an exact `0x38` frame: 44 shared-word differences plus one trailing nop, first `+0x44`. Target relocations are `+0xBC` `mathOneFloatRPY` and `+0xC8/+0xE0` `D_800CB290`; candidate identities agree but are all `+4` late. Authenticated calls are `lightDefaultObjectLight+0x38` and `func_8001A008+0x74/+0xC4`. Only V0 is artifact-backed. Reproduce V0, retain the flag lattice, try two mutually exclusive value-delta-first forms and one trace-selected form, then one improving-only annotated batch. |
...
| `func_8002B524` | `src/main/memory.c` | none | — | Retained configured full-TU and isolated current-body C are byte-identical at 102/116 raw/normalized words, frame `0x58`, first `+0xE0`, with all 12 relocation tuples exact. Ten sites are the slot-record/data carrier, two are the lower-bound branch/delay-slot shape, and two use candidate `sp+0x38` versus target `sp+0x3C`. Ordinary linked equality proves `GLOBAL_ASM` only. Run fresh V0, cache-only and split-guard-only probes, combine strict improvements, then one improving-only call-live record; cap five. |
| `func_8002B7AC` | `src/main/memory.c` | none | — | Retained configured full-TU and isolated current-body C are byte-identical at 62 words versus target 63, frame `0x30`, with 62 raw/normalized positional differences from `+0x4`. The type/identity multiset has all 12 records, but no exact offsets: eleven are four bytes early and the first `D_800D20A8` LO16 is twelve bytes early. The historical 11-row aligned claim has no artifact. Reprove V0, then test only the lexical and explicit early-base/later-cursor lifetime handoffs while preserving both `D_800D20A8` pairs. |
| `func_8002BB40` | `src/main/memory.c` | none | — | Retained configured isolated C is 42/72 frameless words, with 30 register-only differences from `+0x8C` and all eight HI16/LO16 tuples exact. The matching configured full-TU result is historical and unretained; the 72/72 ordinary object is `GLOBAL_ASM`, not C. Reproduce V0 and the historical 57/72 JFG index form, then try dead-argument reuse for the slot-count carrier and, only on improvement, the selected index; cap six builds. |
...
| `func_8003A2C8` | `src/main/menu.c` | none | — | Retained prior-layout configured full-TU and isolated C are byte-identical at 27/32 words, frameless, with six exact relocations and differences at `+0xC/+0x10/+0x14/+0x18/+0x20`. Current physical-layout C is unproven. ORT 606 exports it; callers are `func_80027EC0+0x80`, `overlay46ReleaseState+0x70`, and Overlay 60 `+0x156C`. No linked C artifact survives; linked exactness proves fallback only. Forced-color 31/32 and route exhaustion are unretained prose. Run current V0, 119 flags, one allocator trace, at most three improving forms, then one improving-only bounded batch; cap 122 deterministic builds. |
...
| `func_80020D8C` | `src/main/models.c` | none | — | Retained configured full-TU and isolated C agree at 35/48 words, frame `0x8`, no relocations, first `+0x38`. Thirteen register-only sites cover texture-address, frame-value, multiply-result, and loop-count webs. One resident call and eight runtime overlay records across overlays 57, 60, and 82 are authenticated; linked equality proves fallback only. Only V0 has attributable artifacts. Run V0, the retained flag lattice, one allocator trace, then one trace-selected natural form; cap four. |
...
| `func_8005A948` | `src/main/models_5B300.c` | none | — | Retained configured full-TU C compiles the earlier structured-store body at 83/94 raw/normalized words, frame `0x38`, first `+0x40`, with 13 exact static and zero runtime records. Current raw-array p10 is unscored; its reported 91/94 result and broad route outcomes are unretained. Sole caller is `func_8005A7A0+0x104`; linked equality is fallback-only. Run current V0, retain 119 flags, trace once, and try at most one trace-selected natural address-producer/lifetime form; optional improving-only batch, cap 120 deterministic builds. |
...
| `func_8003E8D8` | `src/main/particles.c` | none | — | Retained prior-layout configured full-TU and isolated C are 131/140 words with exact `0x230` size, `0x38` frame, and all ten target tuples; the isolated import omitted inert `-Wab,-r4300_mul`. Current-layout proof is pending. One branch target differs at `+0x38`; eight call-live homes swap `entry`/`result` positions around both calls. Linked equality proves fallback only. Run current V0 and flag lattice, three declaration forms, one improving-only trace form/combination, then improving-only bounded batch. |
...
| `func_80041CE4` | `src/main/particles.c` | none | — | Retained configured full-TU and isolated C are byte-identical at 126/153 words, frame `0x80`, first `+0x48`, with all nine relocations exact; `-Wab,-r4300_mul` is inert for V0. Historical source, flag, trace, and search outcomes are unretained. Preserve V0, retain the flag lattice, try two trace-selected natural forms and their improving-only combination, then one corrected-flag annotated batch. |
| `func_8002EBE0` | `src/main/rcpFast3d.c` | none | — | No ownership change; continue source/codegen work. |
| `rcpClearZBuffer` | `src/main/rcpFast3d.c` | none | — | Retained configured full-TU C from the preceding `D_800D2FAC` declaration state is 103/107 words, exact-sized/frame zero, with four FIFO/web sites at `+0x74..+0x80` and both relocations exact. The isolated ranking adds seven schedule sites. Reprove current-source V0, then one UGEN FIFO trace and at most two indicated forms; park if flat. |
...
| `func_overlay_001_F0000378_184C758` | `src/overlays/o001/overlay_001.c` | none | — | `overlay1FindType5ByKey` retains prior-layout configured full-TU and isolated C at 22/39 raw/normalized words, frame `0x30`, first `+0x1C`, with 17 register-field sites. Runtime proves outbound `+0x14` to resident `func_8000572C` and sole inbound `overlay1TransitionState+0x40`; V0's private proxy is diagnostic and linked equality is fallback-only. Run current V0, identity-correct V1, retain 119 flags, trace once, try at most two independent natural forms and one improving-only combination, then one improving-only batch; cap 123 deterministic trials. |
...
| `func_overlay_001_F00036A0_184FA80` | `src/overlays/o001/overlay_001_tail.c` | none | — | Retained prior-layout configured full-TU and isolated C agree at 36/44 words with frame `0x38`: five records/length stack-home differences and three conditional-store schedule/address differences. Runtime proves one three-argument `GetRomlistInfo` SYMBOL call and three `D_1D8C` LOCAL HI/LO pairs; V0's four-argument alias, `D_1D8CRead`, and literal write expose only five records, and linked equality is fallback-only. Run current V0, identity-correct V1, one retained flag lattice, five independent declaration orders, then one trace-selected residual form; cap eight natural forms plus the lattice. |
...
| `func_overlay_001_F0007B64_1853F44` | `src/overlays/o001/overlay_001_tail.c` | none | — | `overlay1FindBestRecord` retains isolated and prior-layout full-TU C at 18/30 words, frame zero, with one 12-site `a1/a3` carrier exchange and all four runtime tuples. The isolated import omitted `-Wab,-r4300_mul`; current-layout configured proof is pending and linked proof is fallback-only. Run current V0 and three primary declaration/scope forms; stop if flat, otherwise combine strict gains only within a six-run cap. |
...
| `func_overlay_005_F000031C_185B744` | `src/overlays/o005/overlay_005.c` | none | — | Retained prior-layout configured full-TU and selective C agree at 211/233 raw and runtime-normalized words, frame `0x98`, first `+0x9C`, with all 71 runtime sites aligned by offset/type. The 22 sites are the Span1-size carrier, one initialization-order swap, and three shifted stack-home groups. Runtime identity distinguishes ORT 1559/1561 behind `gOverlay5SoundState` and ORT 1565/1568 behind `gOverlay5ScaleValue`; split them before promotion. ORT 1558 exports the function and `overlay18Load+0x14` is its sole inbound. Linked equality proves fallback only. Run current identity-correct V0, 119 flags, one allocator trace, at most four improving forms, then one improving-only bounded batch. |
...
| `func_overlay_007_F0000894_185C71C` | `src/overlays/o007/overlay_007_tail.c` | resolved-plateau | overlay 7 mode switch table | TU-owned `+0x934..+0x950` range clears module growth. Stale configured full-TU/isolated C artifacts are 129/131 after relocation normalization (`+0x44/+0x64` register fields); they predate the corrected `mathRnd` identities, and raw `+0xA4` is the unresolved switch-table LO16 addend. Fresh source-current full-TU proof must retain 23 text plus seven owned switch-table relocations before the six-build carrier ladder. |
...
| `func_overlay_007_F0000CCC_185CB54` | `src/overlays/o007/overlay_007_tail.c` | none | — | Exact 60-word/`0x20`-frame candidate has five raw words, two after relocation normalization at `+0x4/+0x10` (`t7` versus target `t6`), and requires 13 relocations; representation/flag/local-web routes exhausted. Reproof-only V0. |
| `func_overlay_007_F0000DBC_185CC44` | `src/overlays/o007/overlay_007_tail.c` | none | — | Runtime identity repair changes `+0xF4` from the false `+0xCCC` SYMBOL alias to local JUMP target `overlay7ReleaseEntry`; the other calls are `mathRnd`, `func_800031E8`, and `amSndPlay`. Retained isolated evidence is 72 words/`0x30` frame with six raw and three normalized sites (`+0xBC,+0xC4,+0xD4`) across a required 17-relocation surface. Run one configured full-TU/linked V0, then park absent a new narrowing-register mechanism. |
...
| `func_overlay_011_F0001398_1869BE0` | `src/overlays/o011/overlay11UpdateMenu.c` | none | — | Retained prior-layout configured full-TU and isolated C agree at 285/301 raw and 299/301 runtime-normalized words, frame `0x48`; the only executable residual is the reversed call-live store pair at `+0x138/+0x140`, and all 102 runtime `(offset,type)` sites are present. Runtime proves the `+0x8` identity is resident `func_80028F54`; source and metadata rebind are corrected. The sole inbound is overlay-local `func_overlay_011_F0000150_1868998+0x7E0`, with no export. Linked equality proves fallback only. Run current-layout V0, 119 flags, one allocator trace, at most three improving source forms, then one improving-only bounded batch. |
...
| `func_overlay_014_F0000498_186FD70` | `src/overlays/o014/overlay14ResetMode.c` | none | — | Retained pre-identity configured C is 45/56 raw and 49/56 normalized words, frame `0x30`. The shipped surface has 18 runtime records; current source annotates `+0x54` as `overlay14ReleaseOwner`, but compiled pre/postprocess identity proof is pending. Retain current V0 and the flag lattice, then trace and try at most three natural `s3`-lifetime forms; one bounded batch only after a strict gain. |
...
| `func_overlay_019_F0000D78_1875FD0` | `src/overlays/o019/overlay19ClassifyEdge.c` | none | — | Retained configured full-TU and isolated C are byte-identical at 110/120 words, frame zero, first `+0x138`, with zero static/runtime relocations. Six differences are the `$v1`/target-`$t3` carrier and four exchange two final load pairs. The sole authenticated caller is `overlay19FindAdjacent+0xD8`. Linked function/module/ROM equality proves the assembly fallback only. Historical source, trace, forced-color, and permuter outcomes are unretained; reproduce V0 and run the artifact-unrun 119-flag lattice once, then park if canonical flags remain best. |
...
| `func_overlay_020_F0000204_18767DC` | `src/overlays/o020/overlay20UpdateObjectResource.c` | none | — | Retained prior-layout full-TU and isolated C agree at 90/98 raw/normalized words, frame `0x70`, first `+0xB0`. Four count/entries sites and four separate `value0F`/`start` argument sites remain. Runtime calls at `+0x14/+0x30/+0x134/+0x168` resolve to `trackGetTrack`, `func_8000FEEC`, local `overlay20ConfigureResource`, and `sqrtf`; sole inbound is `func_8000AA38+0x2CC`. Only V0 is artifact-backed; linked equality is fallback-only. Run current V0, retain 119 flags, trace once, try one form per web and an improving-only combination, then one ≤2,000-candidate batch; cap 122 deterministic builds. |
...
| `func_overlay_034_F0000000_18811A8` | `src/overlays/o034/overlay34InitStorage.c` | none | — | Retained configured full-TU/isolated C is 45/50 raw and 46/50 normalized, exact `0xC8`/frame `0x30`, with four stack-home sites at `+0x24/+0x30/+0x78/+0x7C` (`sp+0x18` versus target `sp+0x1C`) and eight required relocations; the fifth raw word is the baked `+0xC4` addend. Historical route claims are unretained. Reproduce V0, retain the flag lattice, then one trace-selected source lever; cap four steps. |
...
| `func_overlay_040_F0000690_1886F40` | `src/overlays/o040/overlay40FadeRecords.c` | none | — | Retained configured full-TU/isolated C is 98/101 words with register-only sites at `+0xC/+0x10/+0x24`, frame `0x8`, and all ten runtime BSS tuples; the sole proven inbound identity is resident trap site `func_8000D978 +0x130`, resolved at runtime, and linked proof is fallback-only. Run V0 and two value-origin/declaration probes; stop if flat, five-run hard cap. |
...
| `func_overlay_041_F0001650_1888988` | `src/overlays/o041/overlay41AddSlot.c` | none | — | Retained configured C is frameless and 49/55 words. Four count-web sites exchange `a1/v1`; the `+0xA0/+0xA4` load order also puts the candidate `D_0` LOCAL LO16 at `+0xA4` instead of runtime `+0xA0`. The other pair is SYMBOL-bound `gOverlay41Slots`; only V0 has attributable artifacts. Run V0, the retained flag lattice, multiplication-operand reversal, then one trace-selected natural count-web form; cap four. |
...
| `func_overlay_043_F0001378_188B348` | `src/overlays/o043/overlay43FilterImage.c` | none | — | Retained configured full-TU C is 39/43 words, exact frameless `0xAC` shape with no relocations, leaving four pixel-web sites at `+0xC/+0x10/+0x20/+0x28`. The isolated 37/43 ranking adds a `+0x7C/+0x80` swap because import changed physical line placement. Reproduce V0, take one allocation trace, then try only its selected physical-line/web-birth forms within five steps. |
...
| `func_overlay_059_F0000070_18B87C0` | `src/overlays/o059/overlay59PrepareEntry.c` | none | — | Retained configured evidence is 52/62 raw and 53/62 normalized words, frame `0x28`, with one `v0/a0` delay-slot allocation web. Its six runtime records distinguish local `JUMP` cleanup calls from resident `SYMBOL` acquisition calls. Historical route claims are unretained. Run identity-correct V0, flag lattice, trace, split value/result carrier, and improving-only line/scope probe; cap five. |
...
| `func_overlay_062_F00000D4_18C22F4` | `src/overlays/o062/overlay62Update.c` | none | — | Retained configured evidence is 294 words/frame `0x88`, five raw/four normalized allocation sites, with all 71 runtime tuples exact. Reproduce V0, run the missing 119-flag lattice, then one 20-minute macro-faithful annotated permuter batch; park if flat. |
...
| `func_overlay_068_F0001250_18C83B0` | `src/overlays/o068/overlay68RebuildSecondaryEntry.c` | none | — | Retained configured full-TU and isolated C are 113/122 words with frame `0x40`, first `+0x58`, and candidate alignment to all 19 runtime relocation requirements; no linked C proof survives. Only V0 has attributable artifacts. Run V0 and the flag lattice, reverse the sentinel, then one trace-selected stack-home form, one probe representation, their improving-only combination, and one annotated batch. |
...
| `func_overlay_074_F00000B8_18CBD58` | `src/overlays/o074/overlay74Update.c` | none | — | Retained full-TU and isolated C agree at 94/100 words, frame `0x60`, with six register-only sites, but that artifact includes two invented empty guards and is diagnostic only. The guards are removed; source-faithful current V0 is uncompiled. Eight runtime tuples include two `D_800D3128` pairs; sole inbound is resident `func_8000AEEC+0x34C`, and linked exactness proves fallback only. Compile clean V0, probe `s32 func_8005776C` and `void func_800291B4(void)`, retain 119 flags, then reverse the outer OR once; cap 123 deterministic builds. |
...
| `func_overlay_097_F0000508_18D83A0` | `src/overlays/o097/overlay97InitScale.c` | none | — | Retained prior-layout configured C is 143/144 raw/normalized words, frameless, first/only `+0xD0`, with zero static/runtime relocations; candidate `a1+0x3E` and target `a3+2` are address-equivalent. Current-layout V0 and linked C are unproven; fallback linkage is exact. The three-word isolated ranking omitted `-Wab,-r4300_mul`. Run current V0, `(u8 *)bounds+2`, independently `&bounds->bounds[1]`, then improving-only narrow scope; cap four. Preserve ORT 1194 and resident caller `func_8000AA38+0x7C`. |
...
| `func_overlay_099_F0000638_18D9BE8` | `src/overlays/o099/overlay99BuildHeightGrid.c` | none | — | Retained genuine pre-p8 isolated C is 104/114 words after runtime-relocation normalization, frame `0x28`, with ten register-only sites and all 29 runtime tuples, but its recipe omitted explicit `-Wo,-loopunroll,0`. The current load-order p8 is historically reported at three sites (`+0x154/+0x15C/+0x170`) but its object is absent; linked range/module/ROM equality proves the assembly fallback only. Sole direct caller is resident `func_80038190+0x188`. Run current configured V0, then one dead-argument carrier and only one strict-improvement combination; cap three. |
...
| `func_overlay_099_F0000800_18D9DB0` | `src/overlays/o099/overlay99RenderSortedEntries.c` | none | — | Surviving prior-layout configured full-TU and isolated C agree at 216/233 raw and 218/233 runtime-normalized words, frame `0x148`, first `+0x64`, with ten runtime-backed sites. Eleven normalized differences are `sorted[]`/call-home offsets and four are the command-value/store web. Historical prose reports a later 220/233 result, but its object and every linked C trial are absent. The six diagnostic proxy calls map to `camGetProjZ`, `camGetPtr`, `func_80022E80`, `func_8002AA50`, `mtxf_transform_point`, and `func_80022FD4`. Sole caller is overlay 99 `+0xD78`; linked equality proves `GLOBAL_ASM` only. Run identity-correct V0, 119 flags, four declaration placements, one trace form, then one improving-only batch; cap 125 deterministic builds. |
