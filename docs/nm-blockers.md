...
| `func_overlay_074_F00000B8_18CBD58` | `src/overlays/o074/overlay74Update.c` | none | — | Retained configured full-TU and isolated C agree at 94/100 words, frame `0x60`, with six register-only sites at `+0xC,+0x68,+0x84,+0x8C,+0x90,+0x124`; its eight runtime tuples include two `D_800D3128` pairs, while target-static metadata retains four calls. Sole inbound identity is resident `func_8000AEEC +0x34C`; no linked C proof survives. Run V0, `s32 func_8005776C` query ABI, `void func_800291B4(void)` hit ABI, and reversed outer OR; combine only strict gains, cap five. |
