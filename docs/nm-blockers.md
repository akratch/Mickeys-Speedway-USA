...
| `levelGetCounts` | `src/main/level.c` | resolved-plateau | D_800CF3E0, D_800CF420 | No configured full-TU C object currently survives. The retained isolated import is 251/259 raw and 252/259 address-normalized words, frame `0x58`, with 37 records; four isolated-import store-order sites precede the three-site `$v0`/target-`$a0` web. Git history records an unretained configured V0 at 255/259 raw and 256/259 normalized. Target endpoint `+0x44/+0x50` names `D_800CF420`; candidate C names `D_800CF3E0+0x40`. Linked equality proves fallback only. Run one unchanged configured/linked V0, then park. |
