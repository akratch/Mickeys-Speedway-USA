...
| `func_overlay_001_F00036A0_184FA80` | `src/overlays/o001/overlay_001_tail.c` | none | — | Retained configured full-TU C is 36/44 words with frame `0x38`: five records/length stack-home differences and three conditional-store schedule/address differences. Runtime proves one three-argument `GetRomlistInfo` SYMBOL call and three `D_1D8C` LOCAL HI/LO pairs; the current four-argument alias, `D_1D8CRead`, and literal write are not identity-correct, and linked equality is fallback-only. Run V0, then identity-correct V1; declaration permutations are conditional on V1, cap eight. |
