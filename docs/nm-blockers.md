...
| `func_overlay_099_F0000800_18D9DB0` | `src/overlays/o099/overlay99RenderSortedEntries.c` | none | — | Surviving configured C predates the current field-store order: 216/233 raw and 218/233 runtime-normalized words, frame `0x148`, first `+0x64`, with ten exact runtime-backed sites. Eleven normalized differences are `sorted[]`/call-home offsets and four are the command-value/store web. Historical prose reports a later 220/233 result, but its object and every linked C trial are absent. Sole caller is overlay 99 `+0xD78`; linked equality proves `GLOBAL_ASM` only. Run identity-correct V0, then a bounded `sorted[]` declaration-placement ladder. |
