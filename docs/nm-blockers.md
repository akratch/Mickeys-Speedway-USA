...
| `func_overlay_011_F0001398_1869BE0` | `src/overlays/o011/overlay11UpdateMenu.c` | none | — | Retained configured isolated C is 299/301 after runtime-relocation masking, frame `0x48`, with reversed call-live stores at `+0x138/+0x140` and all 102 `(offset,type)` sites present. No full-TU C object survives. The `+0x8` call is identity-wrong: runtime targets resident `func_80028F54`, not overlay 11 `+0`. Correct identity, take one UGEN spill trace, then try only its source-line/declaration association levers; cap six steps. |
