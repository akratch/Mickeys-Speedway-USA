...
| `func_overlay_001_F0007B64_1853F44` | `src/overlays/o001/overlay_001_tail.c` | none | — | `overlay1FindBestRecord` has retained configured full-TU/isolated identity at 18/30 words, frame zero, with 12 runtime-normalized `a1/a3` carrier sites from `+0x04` and all four runtime tuples; linked proof is fallback-only. Run V0 and three primary declaration/scope forms; stop if flat, otherwise combine strict gains only within a six-run cap. |
...
| `func_overlay_099_F0000638_18D9BE8` | `src/overlays/o099/overlay99BuildHeightGrid.c` | none | — | Retained configured pre-p8 C is 104/114 words after runtime-relocation normalization, frame `0x28`, with ten register-only sites and all 29 runtime tuples. The current load-order p8 is historically reported at three sites (`+0x154/+0x15C/+0x170`) but its object is absent; linked range/module/ROM equality proves the assembly fallback only. Sole direct caller is resident `func_80038190+0x188`. Run current V0, then one dead-argument carrier and only one strict-improvement combination; cap three. |
