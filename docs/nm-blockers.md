...
| `func_overlay_001_F0000378_184C758` | `src/overlays/o001/overlay_001.c` | none | — | `overlay1FindType5ByKey` retained isolated C is 22/39 words, frame `0x30`, with 17 register-only sites from `+0x1C`. Runtime records prove one call at `+0x14` to resident `func_8000572C` and one inbound call from `overlay1TransitionState`; no configured full-TU/linked C proof survives. Reprove unchanged once, then park absent a new allocator mechanism. |
