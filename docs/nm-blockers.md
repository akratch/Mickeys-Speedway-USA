...
| `func_overlay_074_F00000B8_18CBD58` | `src/overlays/o074/overlay74Update.c` | none | — | Retained configured-isolated C is 94/100 words, frame `0x60`, with six register-only sites at `+0xC,+0x68,+0x84,+0x8C,+0x90,+0x124`; its eight runtime tuples include two `D_800D3128` pairs, while target-static metadata retains four calls. Sole direct caller is resident `func_8000AEEC +0x34C`; no full-TU/linked C proof survives. Run V0, `s32 func_8005776C` query ABI, and `void func_800291B4(void)` hit ABI; combine only a strict gain, cap four. |
