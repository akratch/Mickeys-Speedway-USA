...
| `func_overlay_031_F00006B0_187FBD0` | `src/overlays/o031/overlay31InitializeBuffers.c` | none | — | The current guarded declaration-order replay matches lane source `06c1a74d`, but no exact C object from that spelling survives. Retained prior C is 225/245 words with frame `0x40` versus target `0x48`; it has all 54 static sites (16 calls, 19 HI16/LO16 pairs). Exact canonical object/link/ROM evidence proves `GLOBAL_ASM` only, and the unannotated target erases data identities. Owned range is Overlay 31 `+0x6B0..+0xA84`, no padding; ORT 1386 and resident caller `func_80028564+0x5BC` are authenticated. Compile current C and independently decode/prove all runtime identities before promotion. |
