...
| `func_overlay_097_F0000508_18D83A0` | `src/overlays/o097/overlay97InitScale.c` | none | — | Retained fully configured C is 143/144 words, frameless, with zero relocations and one address-carrier word at `+0xD0`; no linked-C artifact survives, while linked equality proves fallback only. The three-word ranking omitted `-Wab,-r4300_mul`. Reproduce V0, then two independent bounds-derived cursor spellings and improving-only narrow scope; cap four. |
