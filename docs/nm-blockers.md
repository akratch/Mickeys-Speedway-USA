...
| `func_80020D8C` | `src/main/models.c` | none | — | Retained configured full-TU and isolated C agree at 35/48 words, frame `0x8`, no relocations, first `+0x38`. Thirteen register-only sites cover texture-address, frame-value, multiply-result, and loop-count webs; this is not one uniform two-register shift. The sole caller passes owner/context in `a3`, which the callee overwrites unused. Linked function/TU/ROM equality proves fallback only. Reprove unchanged once, then attempt only a UGEN-trace-led removal of the two candidate-only temporaries; otherwise park. |
...
| `func_overlay_068_F0001250_18C83B0` | `src/overlays/o068/overlay68RebuildSecondaryEntry.c` | none | — | Retained configured-isolated candidate is 113/122 words with frame `0x40`, first `+0x58`, and 19 runtime-backed relocation tuples. Two flag lattices, mapping/value/probe representations, lifetime variants, and bounded permutation are exhausted; no linked C proof survives. One unchanged configured/linked reproof, then park. |
