...
| `func_overlay_019_F0000D78_1875FD0` | `src/overlays/o019/overlay19ClassifyEdge.c` | none | — | Retained configured full-TU and isolated C are byte-identical at 110/120 words, frame zero, first `+0x138`, with zero static/runtime relocations. Six differences are the `$v1`/target-`$t3` carrier and four exchange two final load pairs. The sole direct caller is `overlay19FindAdjacent+0xD8`. Linked function/module/ROM equality proves the assembly fallback only. Source, trace, forced-color, and bounded-permuter routes are exhausted; the 119-flag lattice is unrecorded. Reproduce V0, run that lattice once, then park if flat. |
