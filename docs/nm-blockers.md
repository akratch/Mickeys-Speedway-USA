...
| `func_overlay_001_F00036A0_184FA80` | `src/overlays/o001/overlay_001_tail.c` | none | — | `overlay1AssignRecordIndex` is exact-size/`0x38`-frame with eight configured full-TU stack-home/schedule differences at `+0x1C,+0x20,+0x30,+0x40,+0x6C,+0x70,+0x74,+0x7C`; five static ELF relocs understate the seven-record runtime contract (one `GetRomlistInfo` SYMBOL call and three LOCAL pairs for BSS `module+0xA16C`). Fresh V0, then the five-rung declaration ladder and an identity-correct symbolic conditional store; cap at p10. |
