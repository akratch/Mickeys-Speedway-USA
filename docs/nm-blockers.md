...
| `func_8002BB40` | `src/main/memory.c` | none | — | Retained true `NON_MATCHING` full-TU and isolated C are byte-identical at 42/72 frameless words, with 30 register-only differences from `+0x8C` and all eight HI16/LO16 tuples exact. The 72/72 ordinary object is `GLOBAL_ASM`, not C. Reprove V0, then try dead-argument reuse for the slot-count carrier and, only on improvement, the selected index; cap eight builds. |
