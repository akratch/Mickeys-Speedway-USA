...
`overlay 11 +0x1398..+0x184C` (`overlay11UpdateMenu`) — 1,204 bytes / 301 words. NON_MATCHING: the bounded retained block-scoped C has the exact `0x48` frame and all 102 runtime relocation offsets/types, but only 282/301 relocation-masked positional words match, first `+0x1C`. Surface proof resolves and agrees on 73 identities; all 26 calls cover 13 typed zero-addend SYMBOL endpoints, while 29 identities and object-scoped ownership of the BSS aliases remain unresolved. The exact linked range is fallback assembly only and contributes 0 exact C bytes.
...
`overlay 71 +0x0278..+0x07A8` (`func_overlay_071_F0000278_18C9D98`) contributes **1,328 exact C bytes / 332 words**. Reordering only the existing semantic locals restores the retail point-array, cursor, state, index, and floating-point spill homes in the unchanged `0xE0` frame. All six runtime relocation offsets, types, and effective identities agree, including the local resource and global-flag pairs plus the resident calls; the linked owner, complete overlay, and full US ROM are byte-identical.
