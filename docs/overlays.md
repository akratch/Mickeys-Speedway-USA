...
`overlay 34 +0x040C..+0x0540` (`overlay34UpdateRecords`) — **308 bytes / 77 words of unguarded source, but not exact C**. Its configured wrapper still drops three instructions and then invokes `normalize_elf_instructions.py` for instruction reordering and field edits. Any linked-range or full-ROM equality is therefore reached only after prohibited instruction editing and earns no exact C credit. The atlas's `nonmatching:false` and the postprocess audit's `metadata` classification are incorrect.
