...
| `func_80012574` | 0xE4 / 57 words | JFG's assembly-only `trackSphereIntersect` corroborates the role/structure only. Genuine configured full-TU C survives at **50/57 raw and normalized words**, first `+0x50`, frame `0x48`, with the exact `sqrtf` relocation at `+0xA4`; ordinary output is fallback. Two dead stack-home assignments make that score diagnostic and are removed, so clean V0's score/frame are unknown. Five retail calls come from `func_80011CDC` (two) and `func_800563B4` (three); ORT 308 has no runtime/overlay/pointer inbound. | Compile clean V0, run 119 flags, capture one allocation trace, and try at most two trace-supported natural forms plus an improving-only combination. Cap 122 builds plus trace; permit one 20-minute stack-aware batch only after a legal gain. |
