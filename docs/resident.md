...
| `0x80055B24` | `0x1E4` | no unique `hit.c` candidate | tier-D; bounded `structure-mismatch` plateau: exact 121 instructions/`0x50` frame and nine executable call relocations, with 8 positional words remaining (first `+0x20`). Pointer declaration/order, timer lifetime/spelling, and second-state-alias probes were inert or regressed the frame. Mechanism remains the initial pointer-load schedule plus three-use `0x258` temporary in candidate `v1` versus target `v0`; next lever needs target-backed lifetime evidence. |
