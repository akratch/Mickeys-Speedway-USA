...
| `func_8000DDE4` | 0x1D8 / 118 words | Retained isolated current-body and pre-current-layout configured-path full-TU C agree at 94/118 raw/normalized words, frame `0x28`, with both calls exact; this proves the isolated import's omitted `-Wab,-r4300_mul` inert. No current-HEAD selective producer/link proof survives and ordinary equality is fallback. Sole caller is `func_8000E5EC+0x288`; runtime record 157 resolves `+0x128` through ORT 1315 to Overlay 21. There is no export/runtime inbound or target padding. Earlier 15-word claims are unretained. `TrackKeyRecord` is layout-compatible with `TrackRouteResult` (`key`/`sortValue`/padding correspond to `segmentIndex`/`flags`/`object`). | `+0x24`: 24 register-only sites form the bijection (`t1->v0`, `a0->a3`, `v1->a0`, `v0->v1`). Retain one skeleton scan, run 119 flags and one trace, then merge the cross-phase `recordIndex`/`passCount` carrier; continue to `key`/`nextValue` only on strict improvement. Cap 121 builds plus trace and a gain-gated short batch. |
...
`rcpClearScreen`. Historical same-body configured full-TU C under the preceding
`s32 D_800D2FAC` declaration was reported at 103/107 words and frameless, with
one four-site FIFO from `+0x74` and the target HI16/LO16 pair at `+0x00/+0x04`;
no candidate object/report/hash survives. The generated isolated scalar records
96/107, first `+0x74`, but its object is absent and it predates the current
`u16 *` declaration. Current C score/shape/tuples are unknown. ORT 765 exports
the function to Overlay 60 `+0x2194`, Overlay 91 `+0x518`, and Overlay 99
`+0xC8C`; the resident caller
is `rcpClearScreen+0x48`. Exact linked range, resident image, and ROM evidence
all use the assembly fallback, not C. A retained structural scan finds JFG's
assembly-only same-name function first at 0.482 (472 versus 428 bytes), not a C
donor. Compile current V0; use one prior-s32 control only on regression, then
119 configurations including V0, one trace, two trace-supported forms, and a
strict-gain combination. Cap 122 deterministic builds plus trace, 123 only for
the control; historical source/trace/permutation outcomes have no retained
result set.
...
| `func_80047CD8` | `FxGfx **`; `FxCone` words `+0..10`, bytes `+14..17,+2C..32`; `func_800349A4`. | Retained prior-line-layout configured full-TU C is 226/234 raw/normalized words, frame `0x68`, first `+0x298`, with four exact call relocations. Five residual sites are the `cone->mode` value web and three the `segmentCount` web. The current-layout V0, allocator trace, flag lattice, and historical search outcome are unretained; linked equality proves fallback only. |
