...
| `func_80012574` | 0xE4 / 57 words | JFG's assembly-only `trackSphereIntersect` corroborates the role/structure only; Mickey establishes the `0x48` frame. Historical configured C measured **50/57 words** with the exact schedule and sole `R_MIPS_26 sqrtf` relocation at `+0xA4`, but no configured object survives; the retained isolated current-body object omitted `-Wab,-r4300_mul` and is only 55 words. Five retail calls come from `func_80011CDC` (two) and `func_800563B4` (three). A scratch-only empty-condition probe reached **55/57** but is rejected because invented guards are forbidden. | Reproduce configured V0 and its first `+0x50` mismatch. In the rejected two-word probe, only the call-crossing projection spill remains at `+0xA8`: store/reload use `sp+0x30` instead of `sp+0x38`. Run the bounded saved-local/declaration ladder; reject any size/frame/schedule/relocation regression. |
...
| `func_8000DDE4` | 0x1D8 / 118 words | Retained isolated current-body C is 94/118 words, frame `0x28`, with both call relocations exact, but its import omitted `-Wab,-r4300_mul`; no configured full-TU C object survives and current full-TU equality is fallback. The earlier 15-word claim has no surviving source/object and is historical only. `TrackKeyRecord` is layout-compatible with `TrackRouteResult` (`key`/`sortValue`/padding correspond to `segmentIndex`/`flags`/`object`). | `+0x24`: 24 register-only sites form the bijection (`t1->v0`, `a0->a3`, `v1->a0`, `v0->v1`). Fresh configured full-TU/isolated V0, then merge the cross-phase `recordIndex`/`passCount` carrier; continue to `key`/`nextValue` only on strict improvement. |
