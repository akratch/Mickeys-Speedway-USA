...
| `0x80055B24` | `0x1E4` | no unique `hit.c` candidate | tier-D; bounded `allocation-mismatch` plateau: canonical source remains NON_MATCHING; the retained source-faithful pointer-assignment order reaches 118/121 instructions at the exact `0x50` frame, nine executable call relocations, and exact FP/opcode schedule, with three register-only words remaining (first `+0x60`, target `v0` versus candidate `v1` for `0x258`). A faithful five-minute single-worker permuter sweep reached score zero only by inserting a prohibited dead `if (0)` guard after the first call; honest scope and expression-boundary probes remained at three words, while prior function-scope, alias, width, initializer/direct-store, register-qualified, and wrapper-compatible `-g0` probes were inert or regressed the frame. No exact promotion. |
