...
| `func_800056A4` | Shape-exact 19-instruction candidate with 17/19 words matching, frameless, and exact relocation layout | function `+0x24` | Hoisting the loaded table into a natural entry carrier fixes six sites. The remaining two words are one `v1` versus target `t7` pool-to-temp web; a reseeded bounded permuter sweep was flat at score 10. |
