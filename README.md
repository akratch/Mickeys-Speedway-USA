...
[![functions](https://img.shields.io/badge/functions_matched-1140_of_1461_(78.03%25)-blue)](#progress) [![bytes](https://img.shields.io/badge/code_bytes_resolved-399236_of_944416_(42.27%25)-blue)](#progress) [![names](https://img.shields.io/badge/symbols_named-1386_adopted-blue)](#progress)

```
functions     1140 / 1461    78.03%   matched to C, byte-identical
.text bytes 254052 / 475152  53.47%   matched C in the resident segment
verified asm  17184 / 475152   3.62%   original hand-written assembly (84 functions)
overlay C   128000 / 469264  27.28%   matched C keyed by overlay and offset
whole resolved 399236 / 944416  42.27%   resident C + verified asm + overlay C
named         1242 / 1461    85.01%   functions carrying an adopted name
symbols       1386                    adopted in symbol_addrs.us.txt
```
...
```
decompiled              382052 / 944416  (40.45%)
handwritten asm          17184 / 944416  ( 1.82%)
GLOBAL_ASM remaining    215964 / 944416  (22.87%)
NON_MATCHING            329216 / 944416  (34.86%)
NON_EQUIVALENT               0 / 944416  ( 0.00%)
```

| Area | Functions | Matched to C | Named, still asm | Unnamed | Identified |
| :--- | ---: | ---: | ---: | ---: | :--- |
| libultra corridor | 301 | 271 | 29 | 1 | `██████████████████▓▓` 99.7% |
| game code, TU identified | 1153 | 869 | 73 | 211 | `███████████████▓░░░░` 81.7% |
| game code, not yet split | 7 | 0 | 0 | 7 | `░░░░░░░░░░░░░░░░░░░░` 0.0% |
| **total** | 1461 | 1140 | 102 | 219 | `████████████████▓░░░` 85.0% |
...
**Source organization**: 603 fully-C translation units and 256 C scaffolds that still include assembly: 152 under `src/libultra/`; 63 under `src/main/`, including `matrix.c` (matrix/vector maths) and `runlink.c` (the runtime overlay linker core); 644 across 106 `src/overlays/oNNN/` directories (largest: `o101` 71, `o014` 28, `o011` 26, `o057` 22, `o084` 22).
