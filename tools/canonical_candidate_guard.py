...
After a byte-identical build, ``write`` records hashes for canonical objects
whose translation units contain guarded NON_MATCHING bodies.  Before the next
verification, ``dirty`` prints only objects that are missing or no longer
match that receipt.  Make then forcibly recompiles that small set without
trusting timestamps.  A missing or malformed receipt fails safe by selecting
