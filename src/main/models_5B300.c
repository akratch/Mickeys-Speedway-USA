 ...
 * relocations: 10
 * first-mismatch: +0x0
 * summary: 2026-09-11 lane p7-big names both halves from uopt's own records. The 0x50/0x38 frame gap is exactly six declared-local homes: in this function frame = round8(52 + 4N) over the declaration count N, fitted on seven points, and the rule counts an unused s32 while sibling scopes do not share, so the target declares at most one local. The other eight words are p1 web 34, the pre-loop alignment value, identified by force-and-diff: save 1.0, nocs 3, totalsave 3.0, bestcost 2.0, coloured c2 (v1). Its available set omits c14 (s0) because the loop byte-offset web 32 (totalsave 72.0) is coloured first, holds s0 and shares the loop-setup block with it, so the target's s0 requires those two webs not to interfere. Forcing the split is refuted at 65 words delta +4; 39 L97 region placements either leave 10 or cost two instructions; 877 declaration partitions floor at the incumbent. 2026-09-12 lane p8-close reproduced 10 at delta 0 and frame 0x50 and spent its budget elsewhere; the reopen condition above is unchanged and no axis was added. The contested colour c14 is s0, a callee-saved register, so by the 2026-09-12 correction to L142 the call-arity lever cannot reach it and a pass spent on arity here would be inert.
