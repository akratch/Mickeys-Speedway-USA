 ...
 * summary: 13 words in three clusters of one class: the target leaves each computed value on the ugen fp ring and owns the magnitude test's zero on a low fp pool colour, and the candidate does the reverse. This pass turns that description into arithmetic. The fp pool ladder is c24 to c29 = $f0, $f2, $f12, $f14, $f16, $f18, verified by forcing every fp web to every colour, and ugen's fp ring is $f4, $f6, $f8, $f10 plus whichever ladder registers no web claims -- five wide at this base because c25 ($f2) is claimed. The contested zero is uopt web 203, a float constant web with totalsave 1.0, cost 3.0 at every available caller-saved colour, best colour c26, and c24, c25 and c28 forbidden through interference with the zero-fill web ($f0), the value3C web ($f2) and the offset[2] web ($f16); globalcolor colours only when cost is below totalsave, so this web is split and ugen hands it a ring temp. Reaching the target therefore needs two conditions at once: the value3C web must stop interfering so c25 is free, and totalsave must exceed the cost, which means either four references to that constant or a live range that crosses no call. No measured spelling supplies either, and a force cannot: the web is never coloured, so there is no colour to move. New and reusable: a uopt region boundary does move the fp pool-ring split -- a do-while-zero region opened at the sqrt statement frees $f16 and widens the ring to six -- so L97 has an fp form; but 21 region placements were measured and none frees $f2, best 89. Flat this pass: un-naming the value3C load (31), un-naming the Z accumulate (byte-identical), the inlined x-first sum (19), and all sixteen combinations of those three with the region lever, floor 13.
